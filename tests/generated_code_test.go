...
func TestGeneratedCode(t *testing.T) {
	...
	for _, fileName := range inputFiles {
		t.Run(fileName, func(t *testing.T) {
			...
			assert.NoError(t, os.RemoveAll(inputDir+"/generated_pkgsplit"))
			assert.NoError(t, p.WriteOutput())
			// assert.NoError(t, fileutils.CopyDir(inputDir+"/generated_pkgsplit", inputDir+"/expected_output"))
			// diff, err := fileutils.Diff(inputDir+"/generated_pkgsplit", inputDir+"/expected_output")
			// assert.NoError(t, err)
			// fmt.Println(diff)
		})
	}
...
// 		err = os.RemoveAll(fmt.Sprintf("%s/generated_pkgsplit", tc.InputDir))
// 		assert.NoError(t, err)
